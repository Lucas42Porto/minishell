...
/*   Updated: 2024/08/13 21:08:34 by resilva          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	last_pwd(char *pwd)
{
	...
	{
		str = ft_strjoin(getenv("HOME"), &pwd[1]); // Se o diretório atual for home.
...
void	ms_cd(t_shell *shell, t_exec *cmd)
{
	...
	{
		if (!ms_chdir(shell, getenv("HOME"))) // Se o diretório home não existir.
	...
	else
	{
		...
		{
			if (!ms_chdir(shell, getenv("OLDPWD"))) // Se o diretório antigo não existir.
			{
				print_error(shell, "cd", "OLDPWD not set", 1); // Imprime um erro.
			}
			last_pwd(getenv("PWD")); // Imprime o diretório atual.
		}
